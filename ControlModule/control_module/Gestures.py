...
class ArmGestures:
    ...
    def execute_gesture(self, name: str, base_joints: Optional[List[float]] = None, plan_only: bool = False) -> bool:
        ...
        if fn is None:
            ...
            return False
        return fn(base_joints=base_joints, plan_only=plan_only) if name_clean != 'home' else fn(plan_only=plan_only)

    def nod(self, base_joints: Optional[List[float]] = None, plan_only: bool = False) -> bool:
        """
        ...
        self.move.node.get_logger().info(
            f"Starting gesture 'nod' ({'plan_only' if plan_only else 'execute'})")

        for joint_target, label in sequence:
            if not self.move.go_joint(joint_target, plan_only=plan_only, label=label):
    ...
    def shake(self, base_joints: Optional[List[float]] = None, plan_only: bool = False) -> bool:
        """
        ...
        self.move.node.get_logger().info(
            f"Starting gesture 'shake' ({'plan_only' if plan_only else 'execute'})")
    ...
    def search(self, base_joints: Optional[List[float]] = None, plan_only: bool = False) -> bool:
        """
        ...
        self.move.node.get_logger().info(
            f"Starting gesture 'search' ({'plan_only' if plan_only else 'execute'})")
    ...
    def home(self, plan_only: bool = False) -> bool:
        """Move arm to predefined HOME_POSITION."""
        return self.move.go_joint(HOME_POSITION, plan_only=plan_only, label="home")
