...
def _topic_name(spec) -> str:
    """TopicSpec -> topic string, trying the usual field names."""
    ...
    def __init__(self):
        ...
        for name, spec in cam_topics.items():
            ...
            try:
                ...
                self.get_logger().warn(f"[{name}] no intrinsics preset")
            ...
            self._subs.append(TopicHandlerSubscriber(
                ...
                callback=self._make_image_cb(name), qos=10))
    ...
    def _make_image_cb(self, name: str):
        def cb(msg: Image):
            try:
                ...
                self.get_logger().error(f"[{name}] cv bridge: {e}")
                return
 
            detector = self.detectors[name]
            detections = detector.detect(
                img, estimate_pose=True,
                max_reprojection_error_px=self.max_reproj)
 
            h, w = img.shape[:2]
            # Reuse the IMAGE stamp, not now(). The world-space node syncs on
            # this, so it must mean "when light hit the sensor". Stamping at
            # publish time folds detection latency into the sync window.
